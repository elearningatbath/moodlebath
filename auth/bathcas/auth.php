 ...
 */
class auth_plugin_bathcas extends auth_plugin_ldap {
    /**
     * Constructor.
     */
    ...
    function loginpage_hook() {
        ...
        $site = get_site();
        $CASform = get_string('CASform', 'auth_bathcas');
        ...
        if ($this->config->multiauth) {
            ...
            if ($authCAS != 'CAS' && !isset($_GET['pgtIou'])) {
                ...
                echo $OUTPUT->header();
                include($CFG->dirroot.'/auth/bathcas/bathcas_form.html');
                echo $OUTPUT->footer();
                exit();
            }
        }
    ...
    function validate_form($form, &$err) {
        ...
        if ($form->certificate_check && empty($certificate_path)) {
            $err['certificate_path'] = get_string('auth_bathcas_certificate_path_empty', 'auth_bathcas');
        }
    }
    ...
    function ldap_find_userdn($ldapconnection, $extusername)
    {
        ...
        $ldap_contexts =  $this->config->contexts;
        if (!empty($this->config->create_context)) {
            array_push($ldap_contexts, $this->config->create_context);
        }
        $ldap_result = ldap_list($ldapconnection,$ldap_contexts, "uid=".$extusername, $attributes);
        if (!$ldap_result) {
            return false; // Not found in this context.
        }
        ...
        //return parent::ldap_find_userdn($ldapconnection, $extusername); // TODO: Change the autogenerated stub
    }
    function get_userinfo($username,$samis_enrol_plugin = false) {
        ...
        array_push($search_attribs,'accountstate','accesscontrol');
        ...
        $registered = true;
        foreach($user_entry as $key => $arrLDAP) {
            foreach ($arrLDAP as $attributeKey => $attributeValue) {
                    if ($attributeKey == 'accesscontrol') {
                        foreach ($arrLDAP[$attributeKey] as $ACFlag => $ACValue) {
                            if (preg_match('/Moodle/', $ACValue)) {
                                $moodleaccess = true;
                            }
                        }
                    }
                    if ($attributeKey == 'accountstate') {
                        foreach ($arrLDAP[$attributeKey] as $ASFlag => $ASValue) {
                            if (preg_match('/NotRegistered/', $ASValue)) {
                                $registered = false;
                            }
                        }
                    }
        ...
        if(!$moodleaccess){
            if(!$samis_enrol_plugin){
                global $CFG,$PAGE,$OUTPUT,$USER;
                $PAGE->set_url('/login/index.php');
                $PAGE->set_context(context_system::instance());
                $PAGE->set_pagelayout('standard');
                $PAGE->set_title('User status');
                $PAGE->set_heading('CAS Login');
                echo $OUTPUT->header();
                if($registered === false){
                    echo $OUTPUT->heading('Not Registered');
                    echo get_string('not_registered','auth_bathcas');
                }
                else{
                    echo $OUTPUT->heading('You do not have permission to use Moodle');
                    echo get_string('registered_no_moodle','auth_bathcas',$extusername);
                }
                echo $OUTPUT->footer();
                die();
            }
    ...
    function sync_users($do_updates=true) {
        if (empty($this->config->host_url)) {
            error_log('[AUTH BATHCAS] '.get_string('noldapserver', 'auth_bathcas'));
            return;
        }
