...
$settings->add(new admin_setting_configselect('block_sits/sits_gui_enabled', get_string('sits_gui_label', 'block_sits'),
get_string('sits_gui_desc', 'block_sits'), 1, $options));

$settings->add(new admin_setting_configtext('block_sits/sits_disable_message', get_string('sits_disable_label', 'block_sits'),
...
$settings->add(new admin_setting_configselect('block_sits/sits_cron_select', get_string('sits_cron_label', 'block_sits'),
...
$settings->add(new admin_setting_configselect('block_sits/sits_remove_orphans', get_string('sits_orphan_label', 'block_sits'),
...
$settings->add(new admin_setting_configselect('block_sits/sits_hour_of_sync', get_string('config_runtime_label', 'block_sits'),
get_string('config_runtime_desc', 'block_sits'), 1, $options));

$days = array(1=>'Monday', 'Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday');
$settings->add(new admin_setting_configmulticheckbox('block_sits/sits_cron_days','Days to run cron on','Select the days you want the cron to run',array(),$days));
//Only add the settings if the local plugin exist
if(file_exists($CFG->dirroot.'/local/moodle_staff_area_enrolment/lib.php')){
	$settings->add(new admin_setting_configtext('block_sits/sits_moodle_staff_area', get_string('sits_moodle_staff_area_label', 'block_sits'),
