 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignfeedback_offline_bath_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'assignfeedback_offline_bath'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if (!$skip) {
                ...
                $updates[] = get_string('gradeupdate', 'assignfeedback_offline_bath',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'assignfeedback_offline_bath',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'assignfeedback_offline_bath'));
        }
        ...
        $mform->addElement('hidden', 'plugin', 'offline_bath');
