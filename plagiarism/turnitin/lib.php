        ...
        if (!empty($plagiarismsettings['turnitin_attempts']) && is_numeric($plagiarismsettings['turnitin_attempts'])
            ...
            foreach ($items as $item) {
                ...
                if ($file) {
                    ...
                    if (!empty($pid)) {
                    	mtrace ("PID not empty so sending file to turnitin...");//debug message - Hittesh
                    ...
                    debugging('file resubmit attempted but file not found id:'.$item->id, DEBUG_NORMAL);
        ...
        if ($eventdata->eventtype == "mod_created") {
            ...
            if (!empty($eventdata->pathnamehashes)) {
                foreach ($eventdata->pathnamehashes as $hash) {
                	...
                	if (empty($efile)) {
                	    mtrace("nofilefound! for hash: ".$hash);//Debug messages - hittesh
                	    continue;
...
function turnitin_send_file($pid, $plagiarismsettings, $file) {
    ...
    if ($tiixml->rcode[0] == TURNITIN_RESP_PAPER_SENT) { // We need to return the oid and save it!
       ...
       // debugging("success uploading assignment", DEBUG_DEVELOPER);
        debugging("success uploading assignment for course module ".$plagiarism_file->cm." and userid ".$plagiarism_file->userid, DEBUG_NORMAL); //Debug message - Hittesh
    } else {
        //debugging("failed to upload assignment errorcode ".$tiixml->rcode[0].' '.$tiixml->rmessage);
    	debugging("failed to upload assignment for course module ".$plagiarism_file->cm." and userid ".$plagiarism_file->userid ."errorcode ".$tiixml->rcode[0].' '.$tiixml->rmessage);//Debug message - Hittesh
    }
...
function turnitin_create_assignment($plagiarismsettings, $plagiarismvalues, $eventdata) {
    ...
    if ($tiiclassid) {
        $tii['cid'] = $tiiclassid;
        mtrace('class already exists on Turnitin (Course ID: $course->id)'); //Debug message - Hittesh
    } else {
...
function plagiarism_update_record($cmid, $userid, $identifier, $attempt=0) {
    ...
    if (!empty($plagiarism_file)) {
    	mtrace("Updating file record in turnitins's file table: ".$identifier);//debug message - Hittesh
        //update record.
        ...
        if (!$pid = $DB->insert_record('plagiarism_turnitin_files', $plagiarism_file)) {
            debugging("insert into turnitin_files failed",DEBUG_NOMRAL);//debug message - Hittesh
        }
        return $pid;
    }
    return false;
